...
def make_manifest(data_path, csv_path='data', phase='train'):
    ...
    radiologist_labelled = pd.read_csv(os.path.join(csv_path, 'NIH_radiologist_labelled.csv'))
    ...
    if phase == 'train':
        negative = list(set(train_set['Image Index']) - set(radiologist_labelled['Image Index']))
        positive = list(set(train_set['Image Index']) - set(negative))
        ## train-valid split
        ...
        negative = list(set(test_set['Image Index'][['Pneumothorax' not in item for item in test_set['Finding Labels'].tolist()]]))
        positive = list(set(test_set['Image Index']) - set(negative))
